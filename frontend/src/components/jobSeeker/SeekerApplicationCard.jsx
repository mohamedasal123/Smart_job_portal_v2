      ...
      <div className="flex flex-col md:flex-row md:items-start justify-between gap-4 mb-4">
        <div className="flex items-center gap-4 min-w-0">
          ...
          <div className="min-w-0">
            <h3 className="font-h3 text-h3 text-primary truncate" title={job?.title}>{job?.title || t('dashboardComponents.common.unknownJob')}</h3>
            <p className="font-body-md text-body-md text-secondary truncate" title={job?.company}>{job?.company || t('dashboardComponents.common.unknownCompany')}</p>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mt-6 pt-4 border-t border-outline-variant">
        <div className="flex flex-wrap items-center gap-4 text-sm text-on-surface-variant">
          <span className="flex items-center gap-2">
            ...
            {t('dashboardComponents.common.applied', { date: new Date(application.appliedAt).toLocaleDateString(dateLocale) })}
          </span>
          {matchScore > 0 && (
